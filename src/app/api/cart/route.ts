...
export async function POST(request: Request) {
  try {
    ...
    if (!productId || !quantity || quantity <= 0) {
      return NextResponse.json(
        { success: false, error: "Invalid input. Ensure productId and quantity are provided and valid." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ success: true, cartItem: newCartItem }, { status: 201 });
  } catch (error: any) {
    return NextResponse.json(
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, grandTotal:grandTotal, shippingCost :15000 , totalPrice:grandTotal+15000, cartItems: formattedCartItems });
  } catch (error:any) {
    return NextResponse.json(
